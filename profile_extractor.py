...
Output:
  data/user_profiles.json        full profiles (includes stylometry key)
...
from stylometry_extractor import (
    analyze_sentence_structure,
    analyze_punctuation_personality,
    analyze_function_words,
    analyze_opening_closing_patterns,
    analyze_transition_habits,
    build_stylometry_prompt_section,
)
...
DRIFT_THRESHOLD      = 0.6
VOCABULARY_TOP_N     = 20
MAX_REVIEWS_FOR_LLM  = 8     
MAX_TIPS_FOR_LLM     = 3    
MAX_CHARS_PER_TEXT   = 200   
SAMPLE_HIGH_N        = 5    # to N highest-rated reviews for sample
SAMPLE_LOW_N         = 3     # bottom N lowest-rated reviews for sample
RATE_LIMIT_WAIT_SECS = 660   # 11 minutes default wait on 429
RANDOM_SEED            = 42
CHECKPOINT_WRITE_EVERY = 10   # write profiles to disk every N users
                               # prevents losing all progress on Ctrl+C
...
def classify_tone(texts: list) -> dict:
    ...
    avg_length    = round(total_words / len(texts), 1) if texts else 0
    median_length = round(statistics.median(review_lengths), 1) if review_lengths else 0
    ...
    return {
        ...
        "median_length_words": median_length, 
        "tone":                tone,
...
# ─── LLM: MERGED TASTE + STRUCTURAL DNA ───────────────────────────────────────
...
def extract_taste_signals_llm(
    ...
    """
    Single LLM call extracting both taste_signals AND structural_dna.
    Returns (taste_signals, structural_dna, status).
    Heuristic stylometry (sentence structure, punctuation, etc.)
    runs separately for free — only the LLM-dependent structural DNA
    is included here.

    Rate limit handling:
    - Catches 429 and 503 errors
    - Parses retry-after time from error message
    - Waits and retries with exponential backoff
    - Returns fallbacks if all retries exhausted
    """
    ...
    if not all_texts:
        return _fallback_taste_signals(), _fallback_structural_dna(), "fallback"

    prompt = f"""Analyse this user's food review history and extract two profiles.

{chr(10).join(all_texts)}

Return ONLY valid JSON with exactly these two top-level keys — no preamble, no markdown:
...
{{
  ...
  "structural_dna": {{
    "narrative_mode": "story_driven | evaluation_driven | mixed",
    "narrative_mode_explanation": "one sentence",
    "semantic_priority_order": ["what they mention first, second, third — e.g. atmosphere, food, service, value"],
    "emotional_calibration": "overhype | balanced | understated | ironic",
    "emotional_calibration_explanation": "one sentence",
    "sentence_rhythm": "punchy_short | long_winding | varied | compound_heavy",
    "review_structure_pattern": "e.g. scene_setting → action → verdict",
    "favourite_adjectives": ["up to 6 adjectives this person overuses"],
    "hedging_style": "heavy_hedger | light_hedger | direct | none",
    "hedging_examples": ["up to 3 example phrases like 'kind of', 'I think', 'pretty much'"],
    "certainty_markers": ["up to 3 certainty phrases like 'definitely', 'absolutely'"],
    "food_descriptor_style": "precise_technical | casual_emotional | sensory | factual",
    "food_descriptor_examples": ["up to 3 example food descriptions from their reviews"],
    "pronoun_preference": "first_person_singular | first_person_plural | second_person | mixed",
    "notable_quirks": ["up to 3 unusual or distinctive writing patterns worth imitating"]
  }}
}}
...
- If you cannot determine dietary preference, set halal_only to null and confidence to unknown."""
    ...
    def call_api():
        return groq_client.chat.completions.create(
            ...
            temperature=0.2,
            max_tokens=900,   # covers both taste_signals + structural_dna
        )
    ...
    def parse_response(response):
        ...
        return (
            parsed.get("taste_signals", _fallback_taste_signals()),
            parsed.get("structural_dna", _fallback_structural_dna()),
        )

    def attempt(retries_left: int, backoff: int) -> tuple:
        try:
            response = call_api()
            taste, dna = parse_response(response)
            return taste, dna, "complete"
...
def _fallback_structural_dna() -> dict:
    return {
        "narrative_mode":                    "unknown",
        "narrative_mode_explanation":        "",
        "semantic_priority_order":           [],
        "emotional_calibration":             "unknown",
        "emotional_calibration_explanation": "",
        "sentence_rhythm":                   "unknown",
        "review_structure_pattern":          "unknown",
        "favourite_adjectives":              [],
        "hedging_style":                     "unknown",
        "hedging_examples":                  [],
        "certainty_markers":                 [],
        "food_descriptor_style":             "unknown",
        "food_descriptor_examples":          [],
        "pronoun_preference":                "unknown",
        "notable_quirks":                    [],
    }
...
def build_user_profile(
    ...
    review_texts = [r.get("text", "") for r in reviews if r.get("text", "").strip()]
    ...
    # ── merged LLM call: taste signals + structural DNA ───────────────────
    llm_status     = "skipped"
    structural_dna = _fallback_structural_dna()
    if use_llm and all_texts and groq_client:
        taste_signals, structural_dna, llm_status = extract_taste_signals_llm(
            reviews, tips, user_id, groq_client
        )
    else:
        taste_signals = _fallback_taste_signals()

    # ── heuristic stylometry (no LLM cost) ───────────────────────────────
    stylometry = {
        "available":          bool(all_texts),
        "sentence_structure": analyze_sentence_structure(all_texts),
        "punctuation":        analyze_punctuation_personality(all_texts),
        "function_words":     analyze_function_words(all_texts),
        "opening_closing":    analyze_opening_closing_patterns(review_texts),
        "transitions":        analyze_transition_habits(all_texts),
...
def main():
    ...
    for uid in tqdm(pending, desc="Building profiles"):
        ...
        if not profile:
            if args.user_id:
                print(f"  [WARN] Profile returned None for {uid}")
                print(f"         User has no food reviews in the filtered business set.")
                print(f"         Confirm this user_id has reviewed food businesses.")
                print(f"         Check: user_reviews.get(uid) has entries in food_biz.")
            continue
        ...
        # write profiles to disk periodically — prevents losing all progress on Ctrl+C
        if len(profiles) % CHECKPOINT_WRITE_EVERY == 0:
            with open(profiles_path, "w", encoding="utf-8") as _f:
                json.dump(profiles, _f, indent=2)
    ...
    for uid, profile in profiles.items():
        ...
        profile_index.append({
            ...
            "has_stylometry":   profile.get("stylometry",{}).get("available", False),
        })
    ...
    if not profiles:
        ...
        print("  · --user_id has no food reviews in the filtered business set")
        print("  · User ID not found in loaded reviews (check ID is correct)")
        print("  · Food category filter too strict for this user's businesses")
        print("  Tip: run with --no_llm first to confirm pipeline works,")
        print("       then add --user_id to test a specific user.")
        print("────────────────────────────────────────────────────────────────")
        return
    ...
    stylo  = sum(1 for p in profiles.values() if p.get("stylometry",{}).get("available"))
    ...
    print(f"  With stylometry:   {stylo:,} ({stylo/len(profiles)*100:.1f}%)")
